...
from gemma import get_code_summary
...
def loading_animation(elapsed):
    """Display a simple loading animation."""
    symbols = ['-', '\\', '|', '/']
    idx = int(elapsed * 5) % len(symbols)
    print(f"\rGenerating summary... {symbols[idx]}", end="")

def get_summary_with_loading(code_text, file_path=None):
    """Get code summary with loading animation."""
    try:
        return get_code_summary(code_text, file_path, loading_callback=loading_animation)
    except Exception as e:
...
def interactive_search(searcher, top_k=5):
    ...
    print("\n=== Semantic Code Search with Gemma Summaries ===")
    ...
    while True:
        try:
            ...
            for i, result in enumerate(results, 1):
                ...
                # Get and display summary from Gemma
                print("\nGetting summary from Gemma...")
                summary = get_summary_with_loading(formatted['text'], formatted['file'])
                print("\nSummary:")
...
def main():
    parser = argparse.ArgumentParser(description="Semantic Code Search CLI with Gemma")
    parser.add_argument(
        "--db-path", 
        type=str, 
