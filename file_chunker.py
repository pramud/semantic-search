...
class TokenBasedChunker(BaseChunker):
    ...
    def chunk_file(self, file_path):
        ...
        try:
            ...
            # Always create chunks based on token count
            chunks = self.create_overlapping_chunks(source_code, file_path)
            
            # If no chunks were created (unlikely), treat the whole file as a single chunk
            if not chunks:
                logging.info(f"No chunks created for {file_path}, treating as a single chunk")
                header = f"File: {file_path}\nFunction: entire_file\nLines: 1-{source_code.count(os.linesep) + 1}\n"
                chunks = [header + source_code]
...
class CppChunker(BaseChunker):
    ...
    def chunk_file(self, file_path):
        ...
        try:
            ...
            # Always parse the file and extract function-level chunks, regardless of file size
            ...
            # If no function chunks were found, treat the whole file as a single chunk
            if not chunks:
                logging.info(f"No functions found in {file_path}, treating as a single chunk")
                header = f"File: {file_path}\nFunction: entire_file\nLines: 1-{source_code.count(os.linesep) + 1}\n"
                chunks = [header + source_code]
...
class PythonChunker(BaseChunker):
    ...
    def chunk_file(self, file_path):
        ...
        try:
            ...
            # Always extract functions and classes, regardless of file size
            chunks = self.extract_python_functions(source_code, file_path)
            
            # If no function chunks were found, treat the whole file as a single chunk
            if not chunks:
                logging.info(f"No functions/classes found in {file_path}, treating as a single chunk")
                header = f"File: {file_path}\nFunction: entire_file\nLines: 1-{source_code.count(os.linesep) + 1}\n"
                chunks = [header + source_code]
